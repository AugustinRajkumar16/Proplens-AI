...
print("Loading SentenceTransformer model... (may download first-time)")
...
def ensure_client():
    persist_path = CODE_ROOT / "chroma_data"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file", help="Path to a single file to index (optional)", default=None)
    args = parser.parse_args()
    if args.file:
        p = Path(args.file)
        index_files([p])
    else:
        files = [p for p in sorted(UPLOADS_DIR.iterdir()) if p.is_file() and p.suffix.lower() in (".pdf", ".txt")]
        index_files(files)
